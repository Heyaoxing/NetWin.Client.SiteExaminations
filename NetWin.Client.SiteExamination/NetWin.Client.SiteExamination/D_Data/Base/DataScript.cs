...
namespace NetWin.Client.SiteExamination.D_Data.Base
{
    ...
    public abstract class DataScript:DataConfig
    {
        ...
        protected const string InsertExaminationItemDetailConfig = @"delete from  ExaminationItemDetailConfig;
                                                                        ...
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (5, 1, 5, '服务器响应时间', 2, '服务器响应时间小于3秒', 1, '服务器响应时间越快，不仅满足用户访问体验需求，也是搜索引擎与网站建立基础的信任度，利于爬取收录，才能更好的将网站优化发挥极致哦！亲，建议您购买合适国内主机，不宜内存与流量过少！','通俗的说就是网站的打开速度最好是在3秒以内');
                                                                        ...
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (12, 2, 12, 'Sitemap.xml', 4, '有', 1, '您尚未设置相应sitemap.xml。sitemap被搜索引擎用来了解网站内部链接层次和结构，确定网站中网站的页面的重要等级。sitemap能让搜索引擎蜘蛛快速了解你网站内部的所有页面URL，从而利于优化。','sitemap中译：网站地图。作用：通知搜索引擎，您的网站上有哪些可以抓取的网页，以便搜索引擎可以更加智能的抓取网站链接。最简单的sitemap形式，就是XML文件。');
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (13, 2, 13, 'iframe',2, '不使用iframe', 1, 'iframe搜索引擎无法识别的亲，里面展示的内容不仅无法识别，还会影响网站的加载速度哟~','iframe是网站浮动的框架');
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (14, 2, 14, 'flash', 2, '不使用flash', 1, 'flash搜索引擎无法识别的亲，里面展示的内容不仅无法识别，还会影响网站的加载速度哟~','flash是网站视频或者是动画。');
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (15, 2, 15, 'title', 1, '<=30', 1, '亲的网站标题太长哟，百度展示页面显示不完整哦，要适当控制一下呢，30字以内最完美了。如下示例:<img src=http://test-tj.xiniu.com/content/images/title.png >','');
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (16, 2, 16, 'js放于body内', 1, 'js不能放于body内', 1, '这样处理的好处是无需担心因页面未完成加载，造成DOM节点获取不到，使脚本报错的问题，而且能避免因脚本运行缓慢造成页面卡死的问题。说白了就是：影响加载速度','');
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (17, 2, 17, 'H1标签', 4, '建议使用在H1标签', 1, '您的网站尚未设置H1标签。H1标签是网页html 中对文本标题所进行的着重强调的一种标签，作用仅次于Title，建议在页面进行添加。','');
                                                                        ...
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (20, 2, 20, 'URL层级3(含)占比', 4, '总体URL层级3（含）以内占比80%', 1, '您的网站url过长，会加大蜘蛛抓取难度，造成收录效率降低，这样会影响网站的收录的哟！','URL层级一般使用斜杠来区分，一个”/”为一层。斜杠后面带有参数也表示一层级。');
                                                                        ...
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (22, 2, 22, '外部链接数量', 15, '外部链接数量>=1000', 1, '友情链接还可以适当增加哟。益友越多，相当于对亲的网站越是认可，搜索引擎也会酌情给予分值。另一方面，多一条网站的入口，能够提高蜘蛛抓取的几率','');
                                                                        ...
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (27, 2, 27, '各关键词出现次数',3, '各关键词出现次数不低于5次', 0, '你的页面中关键词密度不够哦。要加大关键词的密度哦，要不然网站中的主体内容与关键词离散性太大哦，记得加强呢。','');
                                                                        ...
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (31, 2, 31, '图片具备alt和title属性含目标关键词', 3, '图片具备alt和title属性含目标关键词', 0, '您的网站中图片没有属性呢，蜘蛛可是无法识别的哟；为了让蜘蛛能够识别您网站图片中的内容，记得加上alt和title属性哈，包含关键词可是最佳的方式哦~','');
                                                                        ...
                                                                        INSERT INTO ExaminationItemDetailConfig VALUES (35, 2, 35, '垃圾页面占比', 10, '垃圾页面占比不超过5%', 1, '您的网站页面垃圾页面过多呢。垃圾页面对于搜索引擎的友好性极差，这难以提升网站的优化效果。甚至会严重影响到网站的优化，进而被降权或者惩罚。因此呢记得处理一下垃圾页面，要不然百度可不喜欢您的网站了呢。','');";
        ...
        protected const string InsertComputeRuleConfig = @"delete from  ComputeRuleConfig;
                                                                        ...
                                                                        INSERT INTO ComputeRuleConfig VALUES (6, 1, 2, 'compress', '', 100, 1, '_CONTENT_',100);
                                                                        INSERT INTO ComputeRuleConfig VALUES (7, 1, 2, 'expiredate', '',100, 12, '_COUNT_月',100);
                                                                        INSERT INTO ComputeRuleConfig VALUES (8, 1, 2, 'domainsuffix', '', 100, 1, ' _CONTENT_',100);
                                                                        ...
                                                                        INSERT INTO ComputeRuleConfig VALUES (13, 1, 1, 'iframe', null, 200, 1, '_CONTENT_',100);
                                                                        INSERT INTO ComputeRuleConfig VALUES (14, 1, 1, 'flash', null, 200, 1, ' _CONTENT_',100);
